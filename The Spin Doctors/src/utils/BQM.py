...
def multisource_plot(df,source,power_generation,demand_schedule,capacity_factors,gtype):
# Plot results  
    df['status']='status'
    ...
    nuclear = ddf[ddf.source == 'n']
    nuclear['power'] = power_generation[0]*nuclear.status
    
    coal = ddf[ddf.source == 'c']
    coal['power'] = power_generation[1]*coal.status

    hydro = ddf[ddf.source == 'h']
    hydro['power'] = power_generation[2]*hydro.status

    gas = ddf[ddf.source == 'g']
    gas['power'] = power_generation[3]*gas.status

    solar = ddf[ddf.source == 's']
    solar['power'] = power_generation[4]*solar.status

    wind = ddf[ddf.source == 'w']
    wind['power'] = power_generation[5]*wind.status

    gas = gas.sort_values(by='schedule')
    nuclear = nuclear.sort_values(by='schedule')
    solar = solar.sort_values(by='schedule')
    wind = wind.sort_values(by='schedule')
    hydro = hydro.sort_values(by='schedule')
    coal = coal.sort_values(by='schedule')

    #Add in Capacity Factors
    gas['Capacity Factors']=capacity_factors[3]
    gas['power'] = gas['power']*gas['Capacity Factors']
    coal['Capacity Factors']=capacity_factors[1]
    coal['power'] = coal['power']*coal['Capacity Factors']
    nuclear['Capacity Factors']=capacity_factors[0]
    nuclear['power'] = nuclear['power']*nuclear['Capacity Factors']
    hydro['Capacity Factors']=capacity_factors[2]
    hydro['power'] = hydro['power']*hydro['Capacity Factors']
    wind['Capacity Factors']=capacity_factors[5]
    wind['power'] = wind['power']*wind['Capacity Factors']
    solar['Capacity Factors']=capacity_factors[4]
    solar['power'] = solar['power']*solar['Capacity Factors']

    plt.plot(demand_schedule, label='demand')
    plt.bar(gas.schedule.values, gas.power.values, label='gas')
    plt.bar(nuclear.schedule.values, nuclear.power.values,
    bottom=gas.power.values, label='nuclear')
    plt.bar(solar.schedule.values, solar.power.values,
    bottom=gas.power.values + nuclear.power.values, label='solar')
    plt.bar(wind.schedule.values, wind.power.values,
    bottom=gas.power.values + nuclear.power.values + solar.power.values, label='wind')
    plt.bar(hydro.schedule.values, hydro.power.values,
    bottom=gas.power.values + nuclear.power.values + solar.power.values, label='hydro')
    plt.bar(coal.schedule.values, coal.power.values,
    bottom=gas.power.values + nuclear.power.values + solar.power.values+hydro.power.values, label='coal')
    ...
    if gtype == 'sa':
        ...
        plt.xlabel('Hours of the day')
        plt.ylabel('Energy (GWh)')
        plt.title('Leap Hybrid Solver')
    elif gtype== 'qpu':
        plt.xlabel('Hours of the day')
        plt.ylabel('Energy (GWh)')
        plt.title('D-Wave Quantum Annealer')
